...
const performGetQMAddress = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:8112/address`);
...
const performSend = async (msg) => {
  try {
    const response = await fetch(
      // `http://localhost:8112/send?msg=${encodeURIComponent(msg)}`
      `http://localhost:8112/send?msg=15`
    );
...
const performGet = async () => {
  try {
    const response = await fetch(`http://localhost:8112/recv`);
